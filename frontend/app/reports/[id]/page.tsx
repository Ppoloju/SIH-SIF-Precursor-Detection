            ...
            <div className="min-w-0">
              <div className="flex flex-wrap items-center gap-2">
                ...
                {report.is_demo ? (
                  ...
                  <Link
                    href={`/reports?file=${encodeURIComponent(report.source)}`}
                    title={`Imported from ${report.source.replace("upload:", "")} — click to view every report from this file`}
                    className="rounded-full border border-brand-200 bg-brand-50 px-2 py-0.5 text-[10px] font-bold uppercase text-brand-600 transition hover:border-brand-400 hover:bg-brand-100"
                  >
              ...
              <div className="mt-2.5 flex flex-wrap items-center gap-1.5">
                {report.source_id && (
                  <MetaChip
                    label="Actual ID"
                    value={report.source_id}
                    title={`Actual ID — this report's own ID in its source file`}
                  />
                )}
          ...
          <div className="leading-relaxed">
            <b>
              {report.processing_status === "failed"
                ? "AI analysis failed for this row."
                : "This row is stored but has not been analyzed yet."}
            </b>{" "}
...
function MetaChip({
  ...
  return (
    <span
      title={title}
      className="inline-flex items-center gap-1.5 rounded-full border border-brand-100 bg-white/80 px-2.5 py-1 text-[11px] font-medium text-ink-soft"
    >
